...
function detectAndHandle(input, ctx = {}) {
  ...
  if (/(quanti|quanto|totale|riepilogo).*(ricavi|fatturato|incassato)|(ricavi|fatturato).*(oggi|ieri|mese|settimana|anno|inizio anno|dal|da)/.test(t)) return handleRevenueSummary(input, ctx);
...
app.get('/api/health', (req, res) => {
  res.json({ ok: true, service: 'Rural Vet AI backend', version: '8.2.2-human-replies', model: MODEL, openai: Boolean(openai) });
});
...
app.post('/api/vet-ai-chat', async (req, res) => {
  try {
    ...
    const messages = [
      { role: 'system', content: (system || 'Sei Rural Vet AI. Rispondi in italiano, usando i dati gestionali forniti e senza inventare.') + '\nRispondi sempre con un JSON che contenga almeno la chiave reply come testo umano leggibile. Non restituire mai solo oggetti dati grezzi.' },
